...
export class NeonStorageAdapter implements BlobStorageAdapter {
  ...
  async get(pathname: string): Promise<string | null> {
    const rows = await this.sql("SELECT value FROM kv WHERE key = $1", [pathname]);
    if (rows.length === 0) return null;
    return (rows[0] as { value: string }).value;
  }

  async list(prefix: string): Promise<readonly string[]> {
    const rows = await this.sql("SELECT key FROM kv WHERE key LIKE $1", [
      `${prefix}%`,
    ]);
  ...
  async delete(pathname: string): Promise<void> {
    await this.sql("DELETE FROM kv WHERE key = $1", [pathname]);
  }
}
